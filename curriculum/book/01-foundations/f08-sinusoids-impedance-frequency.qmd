...
This draft covers sinusoidal representation, impedance, network analysis,
resonance, frequency sweeps, loading, and filter design. The synthetic sweep
supports analysis practice, not hardware qualification; Lab **L05** provides the
corresponding practical work. See the [reading roadmap](../roadmap.qmd) for
status definitions.
:::
...
Consider a 10.0 k$\Omega$ resistor followed by a 10.0 nF capacitor, with the
output taken across the capacitor. For a 1.000 V RMS sine-wave input, predict the
output at 100 Hz, 1.59 kHz, and 10.0 kHz before calculating. The same components
...
These are calculations, not measurements. The capacitor relation from
[F07](f07-capacitors-inductors-transients.qmd) leads to their impedance and
frequency-response derivation. Source resistance, load, component tolerance, and
probe loading then determine the design margin.

**Sinusoidal steady state** is the condition after startup transients have
decayed while a circuit is driven by a continuing sine wave. A linear
time-invariant circuit then responds at the input frequency, although amplitude
and phase can change [@sedra2020microelectronic; @oppenheim1996signals].
...
- represent one-frequency sinusoidal steady state with RMS **phasors**, complex
  numbers that record sinusoid magnitude and phase, using the $e^{j\omega t}$
  convention;
- derive and use $Z_R$, $Z_C$, $Z_L$, **admittance** (the reciprocal of
  impedance), complex KCL, and complex KVL with declared voltage and current
  references;
...
- plan a safe frequency sweep and make an uncertainty-aware pass/fail decision.
...
The plot in @fig-f08-bode shows the exact normalized response and its
straight-line magnitude approximation. The figure is **calculated** from the
relation in @eq-f08-rc-lowpass-mag-phase by the recoverable standard-library script
...
The time/frequency bridge is not peculiar to capacitors. In the RL circuit shown
in @fig-f08-rl-frequency, $i(t)$ points clockwise and $v_R$ is positive from
...
$Q$ in @eq-f08-complex-power means reactive power and has unit var. In
the quality-factor relation @eq-f08-series-q, $Q$ is dimensionless. Context is
...
7. Repeat a few points after changing probe mode or load. A shift that matches
   @eq-f08-measured-lowpass supports measurement loading; it is not a DUT
   fault.
...
## Calculation cross-check and synthetic diagnosis

### Executable cross-check

The following standard-library Python evaluates the printed relations. It checks
the hand calculation; it is not a circuit simulator or measurement. The
parameter names keep resistance and capacitance visible. See
@sec-evidence-workflow for the distinction among these forms of work.
...
## Worked design: a low-pass response with margin

### Requirements and nominal selection

The design case applies the low-pass in panel (b) at 20–25 °C under these
...
The requirements first constrain the time constant, before a component pair is
selected. For a nominal unity-gain first-order response, the corner requirement
gives
...
Source impedance, load, probe capacitance, component variation, parasitics, and
instrument bandwidth are not cleanup details. Each can move the measured
response away from the ideal prediction. A calculation supports a prediction;
a documented measurement establishes how the tested assembly behaved.
...
the resonance, bandwidth, and quality-factor relations in @eq-f08-resonance,
the bandwidth relation @eq-f08-rlc-bandwidth, and the quality-factor relation
from @eq-f08-series-q; 6 c — the guarded
