...
This chapter presents a complete instructional path: sinusoidal representation,
impedance derivations, worked network and resonance examples, executable
arithmetic, explicitly synthetic sweep data, a filter decision that includes
uncertainty margin, and a full exercise set. It remains subject to technical and
editorial revision. The synthetic data teach analysis but do not qualify
physical hardware; Lab **L05** supplies the aligned evidence. See the
[reading roadmap](../roadmap.qmd) for status meanings.
:::
...
These are calculations, not measurements. We will derive them from the capacitor
relation in [F07](f07-capacitors-inductors-transients.qmd). That relation leads to
impedance and frequency response. We then add source resistance, load, component
tolerance, and probe loading to see whether the design still has margin.
