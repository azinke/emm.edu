...
This draft connects junction physics to diode circuits, dynamic behavior, and
part selection. Numerical examples and synthetic curves are labeled. You perform
the corresponding terminal-curve and recovery measurements in Lab **L06**.
...
An ordinary **diode** is a two-terminal device whose current depends strongly on
the applied voltage and its polarity. Place a silicon signal diode in series
with a current-limited source and a resistor. In an **illustrative**
room-temperature sweep, the current at $+0.20$ V may be too small for a bench
meter to resolve. Near $+0.7$ V, it can reach milliamperes. Reverse the diode
voltage and the steady current becomes small again until the circuit approaches
a reverse limit. The transition is steep, but it is not an exact switch at
$0.7$ V
[@sedra2020microelectronic; @vishay1n4148].

Four variations test whether that initial picture is sufficient:
...
Each description answers a different question. Doping and carrier motion explain
the junction. The symbol fixes terminal references. Curves and equations predict
an operating point, while datasheets characterize manufactured parts and
measurements test the assembled circuit. If these views disagree, check the
operating region, temperature, time scale, parasitics, and chosen approximation.
...
- declare diode terminal references and use the junction equation,
  **incremental resistance** (the local slope $\mathrm dV/\mathrm dI$), or a bounded
  piecewise approximation without confusing one for a universal law;
...
- translate circuit requirements into datasheet parameters, account for
  tolerance and operating conditions, and state an acceptance rule that reserves
...
When p-type and n-type regions meet in one crystal, their concentration
gradients drive electrons from the n side and holes from the p side toward the
interface. Recombination there removes some mobile carriers and exposes
essentially immobile ionized donors on the n side and ionized acceptors on the
p side. This **space charge** creates an electric field directed from the
positive donor ions toward the negative acceptor ions.
...
The drawing in @fig-a01-pn-junction is a spatial cross-section, not a
band-energy plot, and it is not to scale. It synchronizes the physical structure
with the terminal names and references used throughout the chapter. At
equilibrium the shown diffusion and drift contributions cancel in net terminal
current; the arrows do not imply that
...
For an ideal abrupt p–n junction, solving the minority-carrier transport problem
gives the Shockley junction relation under steady conditions, negligible
generation and recombination in the depletion region, low-level injection,
uniform temperature, and negligible series resistance
[@shockley1949junctions; @sze2006physics]. Here **low-level injection** means
that the injected excess minority-carrier concentration remains small compared
with the equilibrium majority-carrier concentration, so the bulk majority
population and electric-field distribution are not substantially altered:
...
The relation in @eq-a01-diode-fit is a compact constitutive approximation, not a
...
Do not extend the reverse-current approximation into breakdown. A packaged
device's actual leakage may be much larger and more temperature-dependent than
the ideal diffusion $I_S$. The exponential relation also does not describe the
breakdown mechanism.
...
The Thévenin equivalent connects the nonlinear device law to an otherwise
linear circuit. In @fig-a01-diode-operating-point, the source network at the
diode port has voltage $V_S$ and resistance $R_S>0$, while $V_D$ remains
positive at the anode and $I_D$ enters it.
...
The relation in @eq-a01-load-line is not a diode law. It contains every
$(V_D,I_D)$ pair
...
The code evaluates the assumed constitutive fit; it does not simulate or measure
a physical circuit. Its explicit parameters make the nonlinear intersection
reproducible.
...
### Planning a terminal sweep

A physical DC sweep records pairs $(V_D,I_D)$, not merely the source setting.
Include the source resistance and meter loading in the circuit description.
Record the diode lot or specimen, mounting, ambient and estimated junction
temperature, source current limit, test points, instrument ranges, calibration
state, raw observations, and uncertainty basis.

Apply the [evidence workflow](../preface.qmd#sec-evidence-workflow) to the same
terminal quantities throughout. Predict the region and order of magnitude, then
choose and calculate with an appropriate diode approximation. Compare the result
with manufacturer data under matching conditions before you measure $V_D$ and
$I_D$ at the named test points. If the results disagree, check temperature,
polarity, resistor tolerance, meter burden, contacts, and self-heating before
you attribute the discrepancy to the diode.

A typical datasheet curve represents manufacturer characterization, not a
guaranteed limit for every unit. The curve generated from @eq-a01-diode-fit is
synthetic. Neither curve measures the bench specimen, even if all three traces
look similar.
...
The circuit in @fig-a01-zener-reference shows why a breakdown reference is a
current-window problem rather than a “5 V component.” Define $V_S$ as the upper source
...
When the polarity is correct, load current passes through the diode. Its forward
drop reduces the voltage available to the load and dissipates power
$P_D=V_FI_F$. A shunt diode across the input takes a different approach: it
creates a low-voltage path under reversed polarity. That path requires an
upstream fuse or current-limiting element that will act safely under the fault.
Without that element, the protection circuit turns a polarity mistake into a
high-current short.
...
The schematics in @fig-a01-optojunctions map the two conversion directions to
elementary circuits.
...
This nominal relation becomes a design interval only after the source,
resistance, and forward-voltage ranges are carried through it. Accordingly, the
opposing extremes are:

- maximum current uses maximum source voltage, minimum resistance, and the
  lowest defensible $V_F$;
- minimum current uses minimum source voltage, maximum resistance, and the
  highest defensible $V_F$.
...
Electrical limits and optical-output specifications usually have different test
conditions. A typical forward voltage is not a guaranteed minimum for a
worst-case current calculation, and a short-pulse luminous-intensity test is not
automatically a continuous-operation claim.
...
therefore a current-generating junction device, not a light-dependent resistor
[@sze2006physics].

Light current, dark current, capacitance, and response time are normally
specified at different combinations of wavelength, irradiance, bias,
temperature, frequency, and load. They cannot be combined as though they
describe one guaranteed operating point.

[A08](a08-optoelectronics-hmi.qmd) develops condition-matched LED and detector
interfaces, optical power, responsivity, ambient-light rejection, isolation,
displays, and human interpretation. A01's handoff is the junction behavior and
the discipline of keeping electrical, radiant, and luminous quantities
distinct.
...
The waveform in @fig-a01-reverse-recovery is a definition aid, not measured
data. It marks the
...
The preceding junction, switching, optical, and breakdown behavior now supports
a broader family map. A flat list would still be misleading because diode names
mix at least four classification axes:
...
The symbols in @fig-a01-diode-family-symbols support recognition, not a complete
taxonomy.
...
| Family or application class | What physically or terminally distinguishes it | Common role and decisive data |
|---|---|---|
...
| Family | Distinctive behavior | Common role and decisive data |
|---|---|---|
...
| Family | Conversion mechanism | Common role and decisive data |
|---|---|---|
...
which mechanism creates the useful terminal behavior and to verify its
advantages and trade-offs in applicable data.
...
| Circuit need | Datasheet data to seek |
|---|---|
...
### Worked design: a rail clamp with stated limits
...
values below come from the stated design calculations.
...
diode sees less than about 6.25 V reverse voltage under these conditions, well
below 75 V. The calculation supports feasibility at the cited
...
### Acceptance test for the rail clamp
...
## Resistance, temperature, polarity, and switching speed

The four opening changes now have condition-specific answers:
...
## Junction behavior across operating conditions
...
- LEDs and photodiodes are junction devices that exchange electrical and optical
  energy. Optical power, luminous response, geometry, bias, bandwidth, and
  temperature require separate specifications.
- Each datasheet number applies under its stated test conditions. Tolerance and
  measurement uncertainty belong in the final decision rule.
...
1. Explain what the vertical axis, horizontal axis, band-edge curves, occupied
   state dots, and transition arrows mean in @fig-a01-energy-bands and in
   the junction-band diagram in @fig-a01-junction-bands. In particular,
   distinguish an electron crossing a junction from an electron changing bands.
