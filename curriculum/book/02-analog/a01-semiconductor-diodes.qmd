...
An isolated atom has discrete electron energies. In a crystal, interaction among
many atoms broadens these into allowed **bands** separated by forbidden energy
ranges. For electronics the important bands are the nearly filled **valence
band** and the higher **conduction band**. Their separation is the band gap
$E_g$. A completely filled band cannot easily change its collective momentum;
mobile conduction appears when electrons occupy available states in a partially
filled band or when unoccupied states exist in an otherwise nearly full band
[@kittel2005solidstate; @sze2006physics].

Thermal energy can raise an electron across the band gap, leaving an empty
valence-band state. The conduction-band electron has charge $-q_e$, where
$q_e=1.602\,176\,634\times10^{-19}$ C is the exact positive magnitude of the
elementary charge in SI. The unoccupied valence-band state behaves collectively
as a mobile positive **hole**. A hole is not a separate elementary particle
stored in the crystal; it is a useful description of many valence-electron
transitions. Conventional current follows positive-charge motion and is opposite
the average electron drift direction [@bipm2019si].
...
- donor atoms contribute electrons that are readily promoted to the conduction
  band, producing **n-type** material whose majority carriers are electrons; and
- acceptor atoms create readily occupied valence-band states, producing
  **p-type** material whose majority carriers are holes.
...
is the **minority carrier**. Temperature, illumination, and injection can change
their concentrations [@sze2006physics].
...
recombination produce a rapidly increasing **forward current**.
...
and therefore has capacitance [@sze2006physics].
...
a guaranteed limit for every unit. A chapter-generated curve from
@eq-a01-diode-fit is synthetic. Neither is a measurement of the bench specimen.
...
@fig-a01-half-wave-rectifier shows the simplest case with a resistive load and no
...
An electron dropping from a higher to a lower energy state can release energy as
a photon; a photon can also create an electron–hole pair if its energy and the
material's allowed transitions permit. Photon energy is
...
1. An n-type region is normally electrically neutral because:
...
2. With $V_D=V_A-V_K$ and $I_D$ defined from A to K, a reverse-leakage operating
   point normally has:
   a. $V_D>0$, $I_D>0$;
   b. $V_D>0$, $I_D<0$;
   c. $V_D<0$, $I_D<0$;
...
3. The statement “a silicon diode drops $0.7$ V” is best classified as:
...
4. A datasheet's reverse recovery time can be compared directly with a circuit
...
5. A rail-clamp diode from a signal node to $V_{DD}$ conducts fault current into
...
6. A photodiode under illumination is most usefully introduced as:
   a. an exact light-dependent resistance;
   b. a fixed voltage source independent of load;
   c. a junction relation with an opposing light-generated current;
   d. an LED operated below threshold.

**Answer key:** 1 b; 2 c; 3 c; 4 b; 5 b; 6 c.

### Retrieval and explanation

1. Explain how diffusion, fixed ionized dopants, the depletion field, and drift
   establish equilibrium at a p–n junction. State which quantities remain in
   motion even when net terminal current is zero.
2. Distinguish majority carriers, minority carriers, and net material charge in
   p-type and n-type regions.
3. Define forward conduction, reverse leakage, and reverse breakdown using the
   declared $V_D$ and $I_D$ references.
4. Explain why an ideal diode is a circuit approximation rather than a physical
   claim that the device literally becomes a wire or an empty gap.
5. Distinguish a typical curve, a guaranteed electrical maximum, an absolute
   maximum rating, and a chapter calculation.
6. Classify each name by what it primarily describes—structure/mechanism,
