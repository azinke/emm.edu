...
This chapter provides a complete instructional path from transistor control
mechanisms and large-signal regions to low-side, high-side, complementary, and
inductive-load switching. Calculated screens and illustrative assumptions are
identified as such; they are not measurements. Physical terminal curves and
switching-waveform evidence belong in aligned Lab **L06**. The chapter remains
subject to technical and editorial revision. See the
...
Connect a transistor between a current-limited supply and a lamp, resistor, or
relay coil. A control signal too small to power the load directly can command
nearly the entire load current. Remove the control and the current nearly stops.
That observable behavior tempts two incomplete explanations: “a transistor is a
switch,” or “a transistor amplifies current.” Neither statement says which
terminal controls which path, which operating region is intended, how much drive
is required, where the load line places the operating point, or what happens
during the transition.

Before reading further, predict four changes:
...
The useful explanation will make the physical device, terminal references,
schematic, equations, datasheet conditions, switching waveforms, and thermal
boundary agree. A mismatch among them is evidence of a wrong operating region,
insufficient drive, stored charge, parasitic inductance, temperature rise, or a
rating used outside its test condition.
...
[F07](../01-foundations/f07-capacitors-inductors-transients.qmd) as
just-in-time support for exponentials, inductor-current continuity, and charge,
...
- separate conduction, transition, control-drive, and leakage loss; screen
  voltage, current, safe operating area, and temperature without multiplying
  unrelated headline ratings; and
- turn a switching requirement into a datasheet-based selection and a guarded
  acceptance test with explicit limits, conditions, loading, and uncertainty.

::: {.callout-note title="Scope boundary — switching here, amplification next"}
This chapter establishes transistor physics, large-signal regions, controlled
current, switching paths, drive, stress, and temperature. It deliberately stops
before complete amplifier design. [A03](a03-bias-small-signal-amplifiers.qmd) is
reserved for DC bias or polarization circuits, bias sensitivity and thermal
...
in A03. These downstream chapters currently define the intended scope but still
require full authoring.
:::
...
In that region, a useful large-signal constitutive approximation is
...
The base current is therefore not a steering current that magically releases an
unbounded collector current. The external collector circuit and the transistor
output characteristics jointly set the operating point.
...
| Family or name | Control and characteristic behavior | Decisive datasheet evidence | Important boundary or trap |
|---|---|---|---|
...
Consider the NPN low-side stage in @fig-a02-bjt-low-side. $R_{\mathrm{OUT}}$
represents the driver's finite output resistance; $R_B$ is mandatory current
limiting. $R_{BE}$ is an optional defined-state and discharge path, not a
substitute for a valid driver.
...
Define $V_{CC}>0$, $R_L>0$, $I_C$ downward through the load and transistor, and
$V_{CE}$ positive at the collector. KVL gives the exact DC source line
...
This chapter establishes the path; converter timing and optimization belong to
S05.
...
Choose a thermal boundary around the transistor and its package. Electrical
power is converted inside the device; heat crosses through package, interface,
heatsink or board, and ambient paths; internal energy accumulates while
temperature changes. At thermal steady state, a simple junction-to-ambient
screen is
...
Estimate first. A 12 V, 360 $\Omega$ coil draws about 33 mA. A forced current
ratio of 10 needs about 3.3 mA base current. A 560 $\Omega$ resistor from a
roughly 5 V output supplies several milliamperes, so the BJT is plausible. A
Schottky diode rated for 40 V reverse voltage and ampere-scale forward current is
plausible at a 33 mA initial flyback current. None of those estimates is yet a
bounded design.

### Adversarial current and drive corners
...
The following standard-library arithmetic reproduces the static corner screens.
It is **executable calculation**, not circuit simulation or measurement.
...
| Opening change | Defensible prediction | Governing evidence and boundary |
|---|---|---|
