...
This chapter develops a complete instructional path from differential gain and
loop gain to closed-loop amplifier functions, frequency shaping, practical
limits, a datasheet-based design, and a guarded acceptance decision. Numerical
records identified as **illustrative** or **synthetic** are not measurements.
Physical evidence belongs in aligned Lab **L07**. The chapter remains subject to
...
Before calculating, predict the result of each change to a non-inverting
amplifier:
...
loop can make that difference small only while the loop has the correct sign,
enough gain, and enough **phase margin** (the angular distance between the loop
phase and $-180^\circ$ where the loop-gain magnitude equals one). The output
must also have an attainable operating point. The useful engineering habit is
therefore:
...
- derive and use inverting, non-inverting, follower, summing, difference,
  transimpedance, low-pass, and practical integrating configurations;
...
- distinguish a feedback amplifier from an open-loop comparator and design
  simple hysteresis with declared thresholds;
...
[A05](a05-sensors-instrumentation.qmd). Current-feedback amplifiers and
specialized RF techniques require their own device methods and are not covered
by the constant gain-bandwidth approximation used here.
...
The schematic in @fig-a04-terminal-references declares the electrical boundary before any
idealization. TP0 is the reference chosen by the surrounding circuit; it is
not normally an op-amp terminal, and it is not automatically earth, chassis,
or a noise-free return. In a dual-supply circuit one commonly has
...
Both relations are definitions. Choose a **quiescent operating point**, the
steady no-signal condition about which we analyze small changes,
$(V_{dQ},V_{OQ})$, and define
...
Equivalently,
$v_o-V_{OQ}=A_{\mathrm{OL}}(v_d-V_{dQ})$. The affine form is essential:
$v_d$ is reference-invariant, whereas the numerical value of $v_o$ changes if
TP0 changes. The equation is not a conservation law and is not valid for
arbitrary output. It describes a bounded linear operating region. The supply
rails deliver the output energy; the input signal controls that delivery.
...
This diagram is a return-loop abstraction, not a replacement for the component
schematic. In an inverting amplifier, $\beta(s)$ is found with independent
signal sources set to zero; it must not be confused with the signed signal gain
from $v_i$ to $v_o$. This chapter uses the conventional return-ratio magnitude
and phase quantity $T=A\beta$: the explicit minus sign at the summing junction
is factored out, so the characteristic equation is $1+T=0$. The signed product
including that displayed subtracting operation would be $-A\beta$.

Consider the scalar low-frequency case. The feedback network returns
...
The circuits in @fig-a04-summing-difference extend the same KCL reasoning. The inverting summer
has input currents referenced from sources $v_1,\ldots,v_n$ toward the summing
node. With the non-inverting input at TP0,
...
input a real op amp needs to produce its nominal balanced output. For
first-order error analysis, we draw it as a differential voltage source.
...
V/$\sqrt{\mathrm{Hz}}$, and let $i_{n-}(f)$ be its inverting-input
current-noise amplitude spectral density in A/$\sqrt{\mathrm{Hz}}$. A
...
Set independent signal sources to zero and calculate the gain from an
equivalent differential error source to output. That is the **noise gain**.
...
The comparison in @fig-a04-capacitive-load shows why the phrase “add an isolation resistor” is
incomplete. Sensing before $R_{\mathrm{iso}}$ leaves the load capacitor
...
The map in @tbl-a04-family-selection is a search aid, not a guarantee: categories overlap,
and the actual supply, common-mode, offset, bias, noise, output, stability,
package, temperature, and lifetime specifications must still be checked in a
current manufacturer datasheet [@horowitz2015art; @sedra2020microelectronic].
...
The design schematic @fig-a04-single-supply-design uses a non-inverting stage whose lower feedback
resistor returns to a 2.50 V reference rather than TP0. The reference must be a
low-impedance, bypassed node over the signal band; two unbuffered equal
resistors are not automatically adequate because feedback current would move
their midpoint. At the input endpoints the feedback network asks the reference
...
Choose $R_G=10.0$ k$\Omega$ and $R_F=30.0$ k$\Omega$, each 0.1%, so the nominal
gain is 4.000. The exact worst-case ratio screen is
...
Before choosing a device, allocate the 20.0 mV DC output-error requirement.
The resistor and reference errors interact. With
...
claiming that separately rounded 6.0 mV and 3.01 mV screens are independent
bounds. @tbl-a04-dc-allocation keeps bounded, illustrative, and unqualified
contributions visibly separate.
...
### Measurement and guarded decision
...
The most useful evidence is a pattern across representations, not one output
number. @tbl-a04-diagnostic-patterns lists discriminating observations. The
entries are hypotheses until the indicated measurement supports them.
...
Every ideal result demands checks of input common mode, differential input,
output swing and current, source and load, offset and bias currents, resistor
tolerance, bandwidth, slew rate, settling, overload recovery, noise gain,
random-noise bandwidth, small-signal distortion, stability, thermal
dissipation, supply decoupling, layout, and measurement loading. Typical
datasheet values establish expectations, not guarantees. A defensible design
keeps schematic, equations, executable arithmetic, datasheet conditions,
physical test points, uncertainty, and decision rule synchronized.
...
13. A flat output-noise density of 40 nV/$\sqrt{\mathrm{Hz}}$ is observed from
