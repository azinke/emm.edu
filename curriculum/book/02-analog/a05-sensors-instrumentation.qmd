...
This chapter presents a complete instructional path from transduction through a
...
the physical event and a defensible result.
...
- If we calibrate only zero and full scale, what evidence would reveal
  curvature between those points?

The chapter answers those questions by keeping the physical object, terminal
behavior, equations, schematic, calibration data, uncertainty budget, and
decision attached to one chain.
...
If voltage excitation doubles while $R$ remains approximately constant,
@eq-a05-self-heating predicts four times the power. This limiting trend explains
why “more signal” can create more measurement error.
...
and attainable bandwidth even though the ideal charge gain in
@eq-a05-charge-amplifier depends on $C_F$. Leakage, dielectric behavior,
...
$R_1=R+\Delta R$, while $R_2=R_3=R_4=R$. With the polarity in
@fig-a05-wheatstone-bridge, a positive $\Delta R$ lowers $v_+$, so predict
$v_d<0$. Substitution in @eq-a05-bridge-general gives
...
magnitude of 2.000 mV/V at 1000 N and uses 5.000 V excitation. We declare its
wiring so positive compression produces positive $v_d$. Its nominal full-scale
...
We want a nominal output from 0.500 V to 2.500 V, so the required span gain is
...
points by construction; only interior points such as those in
@tbl-a05-synthetic-calibration reveal curvature between them.
:::
...
We now connect the chapter's representations. The design input is deliberately
an illustrative transducer requirement rather than an invented product
datasheet.
...
F06 develops propagation in detail. Here we apply it. Let the reported error be
...
multipoint correction; the larger endpoint-fit residuals in
@tbl-a05-synthetic-calibration diagnose why that correction is needed.
...
weakly motivated. If the emulator passes but off-center physical loading fails,
the mechanical installation becomes the stronger hypothesis. The disagreement
between representations is the useful signal.
