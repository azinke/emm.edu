...
The next figure broadens that mechanical view into the complete measurement
chain. It is architectural, not a component schematic: each arrow marks a
transformation that needs a physical or calibration relation, and each
connection can add loading, variation, or uncertainty.
...
Those distinctions lead to the same selection sequence for every candidate:
...
The four-wire interface makes the important connection point explicit: the
sense conductors join at the sensor terminals, after the force-lead
resistances.
...
For an illustrative calculation, let $R=350~\Omega$, $k=2.00$,
...
The complete interface below shows how those constraints coexist. Equal series
elements preserve symmetry, the common-mode and differential capacitors provide
distinct return paths, both amplifier inputs retain DC bias-current paths
through the bridge, and the reference driver establishes the output level.
...
Routing the two signal conductors together makes external fields couple as
equally as possible. Twisting reduces loop area and improves symmetry, but it
does not remove differential pickup or repair input-range violations.
Excitation and high-current returns therefore should not share impedance with
...
Keeping the shield, chassis, protective earth, signal reference, excitation
return, and sensor body as distinct schematic nodes makes every deliberate
connection—and its current path—visible [@ott2009electromagnetic].
