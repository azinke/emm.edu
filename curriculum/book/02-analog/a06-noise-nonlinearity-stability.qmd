...
The candidate paths can be separated by changes that affect one coupling
mechanism more than the others. Rotating or shrinking a loop tests magnetic
coupling; changing source impedance or adding a grounded electrostatic shield
tests electric coupling. Powering the source from a different approved supply
tests conducted coupling, while a differential measurement across the intended
return exposes shared-impedance voltage. A low-pass filter may reduce the
displayed line without proving which path created it.
...
To show how close floor subtraction magnifies uncertainty, assign illustrative
...
The next schematic supplies the circuit context that every noise number needs.
Its source resistance converts op-amp input-current noise into voltage noise.
The feedback resistors generate thermal noise and set **noise gain**, the gain
from an equivalent input-voltage source to the unfiltered output TPU. A
physical post-amplifier low-pass then connects TPU to the observed output TPO;
its resistor adds thermal noise, and its capacitance loads TPU through that
resistor. The supplies deliver output energy but are omitted from this
small-signal noise budget. Their ripple enters through finite supply rejection
and must be added separately when consequential.
...
The time waveform and spectrum provide complementary evidence of clipping. The
conceptual figure shows centered symmetric hard clipping and emphasizes the odd
products added by that symmetry; clipping also changes the fundamental
amplitude. The drawing is explanatory, not device data.
...
Because any one view can hide a stability problem, a defensible assessment uses
at least two of the following evidence views:
...
The right mitigation follows the dominant mechanism, so the design sequence
keeps diagnosis ahead of component changes:
