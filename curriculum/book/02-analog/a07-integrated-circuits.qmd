...
description: "Connect CMOS device geometry, circuit behavior, variation, physical layout, verification, packaging, and test in a bounded integrated-circuit design flow."
chapter-id: A07
status: draft
---
...
verification, and a bounded equation-level decision. Treat every numerical
record here as illustrative. This repository does not yet provide a
version-pinned PDK run with genuine PVT, DRC, LVS, and pre/post-PEX artifacts.
You therefore cannot use this chapter to satisfy the part's principal
pre-silicon evidence contract or qualify a fabrication process. See the
...
integrated circuit may contain protection structures that divert a bounded ESD
...
The first answer is usually the integrated pair because proximity supports
**matching**, similarity between nominally equal devices under stated
conditions. The second has no single “better” direction: current capability and
capacitance both rise. The third is usually slower because extraction reveals
resistance and capacitance omitted from the schematic. **Parasitic extraction**
calculates those unintended physical-network elements from layout geometry and
process data. Each answer is
conditional. A poor layout can make nearby devices systematically unequal, a
designer can divide a wider device into efficient fingers, and coupling can
occasionally
speed one transition while slowing another. The chapter develops the conditions
...
- make a bounded pre-silicon decision from calculations and versioned simulation
  evidence at the available evidence rung without presenting an equation-level
  exercise as PDK extraction, measured yield, or product qualification.
...
Trace the five-transistor **operational transconductance amplifier (OTA)** in
@fig-a07-five-transistor-ota. It converts differential input voltage into
...
behavior, device stress, and the thermal path. “Rail-to-rail” is a bounded
terminal specification under stated load and temperature, not a property that a
...
10. **Apply the decision rule.** Include numerical allowance and any declared
   ...
   strict limit fails. A clean finite Monte Carlo sample remains a bounded
...
different directions. Let the extracted analysis decide each case.
