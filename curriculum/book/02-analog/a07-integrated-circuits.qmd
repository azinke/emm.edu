...
current and capacitance, while nearby devices share wells, substrate,
...
Micrometres of routing add **parasitic effects**—unintended resistance,
capacitance, inductance, or coupling that physical construction creates. A
package then inserts pads,
...
The conceptual cross-section below makes the hidden body connections physical.
The nMOS sits in p-type material, while the pMOS sits in an n-well. Source and
drain regions form p–n junctions to those bodies, so normal bias keeps the
junctions reverse biased. Metal reaches each region through contacts and vias.
The drawing establishes connectivity and polarity; it is not a legal mask
layout or an accurate process cross-section [@sze2006physics;
@skywater2026pdk].
...
Area estimates show why neither component is automatically compact. For an
**illustrative** poly sheet resistance
$R_{\square}=200~\Omega/\square$, a 100 k$\Omega$
...
The CMOS inverter becomes a logic primitive in D01. Here it is analyzed as two
nonlinear transistors sharing an output node. The left panel of
@fig-a07-cmos-inverter-switch shows the pMOS connecting the output toward
$V_{DD}$.
...
The qualitative transfer curve connects those definitions to visible features.
Its two slope markers define $V_{IL}$ and $V_{IH}$, while the intersection with
the $v_o=v_i$ diagonal defines
...
The current relation also leads to an energy balance for a boundary containing
the inverter and load capacitance. Let $i_{DD}$ be positive when it enters that
boundary from the $V_{DD}$ supply.
...
The gate-voltage change that changes off-state current by one decade is
expressed by the **subthreshold swing** $S$:
...
These effects make a transmission gate a controlled analog connection rather
than an ideal wire. In particular, wider switch geometry lowers on-resistance
...
[A03](a03-bias-small-signal-amplifiers.qmd#current-mirrors-provide-conditional-bias-currents)
introduced the simple low-side MOS current mirror: diode-connected reference
device $M_1$ and output device $M_2$ share gate, source, and body potentials.
Integration makes matched geometry and shared temperature practical, but it
does not make current replication exact. Let both devices operate in
saturation, and use the first-order channel-length-modulation relation
...
The gain sign follows from the declared input polarities:
...
To expose the current-steering mechanism, assume identical long-channel nMOS
devices with parameter $\beta$ and neglect channel-length modulation and body
effect. The ideal tail source fixes total current $I_T$:
...
The preceding relations can now be combined into one stage calculation. Its
parameters are
...
The gain and bandwidth results are useful only inside the operating window.
With
...
The two terms describe different mismatch mechanisms. Threshold mismatch
...
another high-impedance node. The incremental two-stage representation in
@fig-a07-two-stage-incremental shows that added node and applies only around a
valid quiescent point.
...
This rate is an output-slew estimate only when one capacitor terminal moves
...
The flow below connects requirements to test. Each arrow means “supplies the
...
Because each stage supports a different claim, the evidence remains separated
as follows:
