...
the applicable product standard [@iec62471_7_2023]. Bare laser
diodes, unclassified optical sources, mains-connected circuits, and tests across
unknown-energy insulation barriers are outside this chapter's boundary
...
flowchart TB
  ...
  X["scene or optical source"] --> G["geometry and spectrum"]
  G --> PD["photodetector"]
...
- bound LED current and resistor dissipation across source, device, and
  ...
  variation;
- derive photodiode responsivity from photon conversion, design a
  transimpedance operating point, and preserve headroom under ambient light;
...
@fig-a08-spectral-weighting shows the operation of the integral. Its schematic
...
Inspect the left side of @fig-a08-led-photodiode-interface. The source,
resistor, LED, switching transistor, base drive, and return all belong to the
current path. Omitting the source tolerance or transistor voltage would change
...
Define $I_F>0$ downward through $R_{\mathrm{LED}}$, the LED, and the transistor.
Let $V_F>0$ be the LED anode-to-cathode voltage and $V_{CE}>0$ the transistor
collector-to-emitter voltage. KVL gives the exact lumped branch relation
...
Consider a 220 $\Omega$, 5% resistor, a 4.75–5.25 V supply, and the cited
...
Let $\eta_{\mathrm{ext}}$ be the **external quantum efficiency**, the average
...
### Photodiode, phototransistor, and photoresistor behavior

These detector names do not describe interchangeable parts:

| Detector | Primary electrical output | Strength | Principal limitations |
|---|---|---|---|
| photodiode | light-generated junction current | linearity, speed, defined responsivity | small current; capacitance, dark current, amplifier noise |
| phototransistor | optically generated base drive produces collector current | internal current gain | wide gain spread, slower response, saturation storage, temperature dependence |
| photoresistor | illumination changes bulk resistance | simple large resistance change | slow and history-dependent response; material and environmental restrictions |

Zero-bias **photovoltaic operation** often favors low dark current. Reverse-biased
**photoconductive operation** can reduce junction capacitance and improve speed
but generally increases dark-current consequences. Reverse bias does not create
the primary photocurrent; absorbed photons do. Device area improves collection
for a fixed irradiance while usually increasing capacitance
[@sze2006physics; @fraden2016sensors].

The BPW34 demonstrates why the conditions stay attached. At 25 °C, 5 V reverse
...
Use the BPW34's typical 50 µA test-point current only as an authentic datasheet
input, not as a guaranteed application current. With $R_F=47~\mathrm{k}\Omega$,
...
Join the earlier illustrative geometry to a narrowband detector with
$\mathcal R_\lambda=0.55~\mathrm{A\,W^{-1}}$. The predicted 3.25 nW received
signal gives
...
Let ambient light produce 8.0 µA, take $R_F=100~\mathrm{k}\Omega$ and
$V_{\mathrm{REF}}=0.20$ V, and suppose the smaller op-amp/ADC upper linear
limit is 3.0 V. Then $V_{\mathrm{off}}=1.000$ V and
...
Separate the received current into signal, ambient, and dark contributions:
...
Read @fig-a08-ambient-modulation from top to bottom. The sample difference is
...
Inspect @fig-a08-optocoupler-interface. The two ground symbols name separate
...
Define input forward current $I_F>0$ through the optocoupler LED and output
collector current $I_C>0$ into the phototransistor collector. The
...
Let the output be accepted low only if $V_O\le V_{OL,\max}$. With output supply
...
Define $I_{\mathrm{RX,src}}>0$ as receiver current entering the output node.
...
Let $C_{\mathrm{node}}$ include receiver, phototransistor, wiring, and probe
capacitance at TPO. After the phototransistor turns off, an otherwise unloaded
...
@fig-a08-multiplex-paths joins the shared current path to the waveforms that
control it. The safe order is: disable every digit, wait for the old switch to
release, change segment data, allow it to settle, enable exactly one new digit,
and hold its active interval.
...
Consider a current-limited, extra-low-voltage monitor with these
**illustrative engineering requirements**:
...
Use the 220 $\Omega$ LED branch derived earlier on the 5 V rail. Its conservative
...
**Answer key:** 1-d, 2-b, 3-c, 4-d, 5-c, 6-b.
...
7. An illustrative LED branch uses a 3.0–3.6 V source, a 150 $\Omega$ ±1%
...
8. A monochromatic 850 nm photodiode has
...
9. A TIA has $R_F=100~\mathrm{k}\Omega$, $C_F=10$ pF, and a 1.20 V output
...
10. A three-digit display uses a 1.5 ms active interval and 0.10 ms blanking
...
11. For $L_{\mathrm{on}}=120~\mathrm{cd\,m^{-2}}$,
...
12. An ideal point source has radiant intensity
...
13. In the VO617A example, explain why the CTR calculation gives 3.15 mA of
...
14. An illustrative LCD differential waveform is +3.0 V for 45% of a frame and
...
15. The VO617A-2 data used in this chapter specify CTR at 5 mA, 5 V, and 25 °C.
...
16. A multiplexed display shows faint segments only during pattern changes.
...
17. A reflective optical sensor works under daylight but fails under one LED
...
18. An LCD looks correct, but the measured average SEG voltage is 1.2 V.
    Explain why this observation neither proves nor disproves DC stress. State
    the correct measurand.

### Open design

19. Specify an accessible indicator for `OFF`, `STARTING`, `READY`, `WARNING`,
...
20. Design a modulated photodiode receiver for an object detector. State source
...
21. Select a display family for a battery instrument that must work in bright
...
- **Downstream use:** [D03 — State, clocks, and
  ...
  timers and display interfaces. [R05 — Metrology, reliability, and
...
[References](../references.qmd). Datasheet evidence applies to the cited parts
