...
def platform_matrix() -> dict:
    return {"schema_version": "1.0.0", "version": "1.1.0", "status": "draft", "platforms": [
        {"id": "PLAT-MCU-FOUNDATION", "category": "mcu-foundation", "capability_contract": "low-voltage foundation MCU with mapped I/O domains, ADC, timer/PWM, UART/SPI/I2C, inspectable memory/resource use, hardware-debug route, reversible bad-image recovery, and archived offline C/C++ plus introductory-language tooling; radio is disabled unless separately approved", "reference": "Raspberry Pi Pico 2 / RP2350-class", "alternatives": ["institution-approved capability-equivalent MCU"], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
        {"id": "PLAT-MCU-PRO", "category": "mcu-professional", "capability_contract": "professional MCU with exact electrical/alternate-function mapping, ADC, timer/capture/PWM, production serial/field buses, measured bare-metal/RTOS memory headroom, SWD/JTAG-class debug, reversible recovery, frozen offline RTOS/toolchain support, and no required irreversible security state", "reference": "deployment-selected", "alternatives": [], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
        {"id": "PLAT-WIRELESS", "category": "wireless-node", "capability_contract": "wireless MCU/module with mapped I/O and peak-power behavior, wired fault visibility and recovery, sufficient radio/security-stack memory, archived offline SDK/RTOS path, demonstrable transmit-disabled state, and dated jurisdiction approval for the exact module, antenna, firmware, region, and configured power before transmission", "reference": "deployment-selected from ESP32-C3/C6-class or qualified Zephyr-supported radio boards", "alternatives": ["Pico 2 W-class after qualification", "nRF52840-class development kit or module after qualification", "approved non-transmitting wired path while radio authorization is pending"], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
        {"id": "PLAT-FPGA", "category": "fpga", "capability_contract": "education FPGA with documented I/O-bank limits, accessible clock/reset and serial/PMOD-style I/O, sufficient logic/memory headroom for the approved architecture artifact, self-checking simulation, inspectable synthesis/place-route/timing reports, offline programming, and reversible known-good-image recovery", "reference": "deployment-selected", "alternatives": ["simulation-first path before board access"], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
        {"id": "PLAT-LINUX-EMBEDDED", "category": "embedded-linux", "capability_contract": "supported SBC or equivalent target with protected mapped GPIO/peripheral interfaces, local console/log diagnostics, replaceable or recoverable storage, measured image/service RAM and storage headroom, reproducible offline provisioning/build, MCU bridge for deterministic timing, and disabled or locally approved radio/network configuration", "reference": "deployment-selected Raspberry Pi 4/5-class or capability-equivalent target", "alternatives": ["qualified BeagleBone/BeaglePlay-class target", "fully imaged and qualified equivalent SBC", "refurbished x86 mini-PC plus qualified remote MCU"], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
        {"id": "PLAT-DEBUG-PROGRAM", "category": "debug-probe-programmer", "capability_contract": "interface-matched reusable probe/programmer with explicit target-voltage sensing and powering limits, keyed/strain-relieved adapters, required SWD/JTAG/UART or FPGA programming operations, offline drivers/server/debug integration, archived firmware and self-recovery, genuine/clone provenance, and no need to place reusable targets in irreversible state", "reference": "deployment-selected interface-matched institution-qualified tool", "alternatives": ["on-board debug probe after exact target qualification", "separately powered institution-qualified probe/programmer"], "binding": "platform-bound", "review_due": "2027-07-13", "context_ids": [], "claim_ids": ["CLM-PLAT-001"], "status": "qualification-pending"},
...
def ese111_ch04_claim_records() -> list[dict]:
    register_source = {
        "type": "unit-claim-register", "id": "ESE111-CH04-CLAIMS", "version": "0.1.0",
        "retrieved": "2026-07-14", "path": "curriculum/courses/ESE111/shared/claims.yml",
    }
    return [
        ...
        {"id": "ESE111-CH04-CLM-007", "wording": "Kirchhoff's 1845 paper is the historical source associated with the circuit rules used in this unit.", "claim_class": "contextual-empirical", "verification_method": "primary bibliographic record review", "source": {"type": "primary-publication", "id": "doi:10.1002/andp.18451400402", "version": "1845", "retrieved": "2026-07-14", "url": "https://doi.org/10.1002/andp.18451400402"}, "assumptions_and_domain": "narrow historical attribution, not a technical proof or priority claim", "affected_assets": ["curriculum/courses/ESE111/shared/claims.yml"], "reviewer": "pending-source-and-license-reviewer", "last_reviewed": None, "review_due": "2031-07-14", "trigger": "bibliographic contradiction or changed historical wording", "context_ids": [], "status": "pending"},
    ]
...
def supporting_records() -> dict[str, dict]:
    return {
        ...
        "safety.yml": {"schema_version": "1.0.0", "version": "1.1.0", "status": "draft", "levels": [
            {"id": "L0", "scope": "observation, simulation, and verified de-energized inspection or assembly", "authorization_gate": None, "supervision": "staff present or an approved low-risk activity plan; energization is not permitted", "stop_conditions": ["unknown energy source", "stored energy not discharged and verified", "damaged apparatus"]},
            {"id": "L1", "scope": "approved current-limited extra-low-voltage core work; planning caps normally no more than 12 V DC, 1 A, 12 W fault power, and 0.25 J accessible capacitor energy, all pending institutional safety review and activity-specific confirmation", "authorization_gate": "G1", "supervision": "at least one trained instructor or assistant per 12 learners; before G1, direct-supervised instructor-controlled stations with instructor-controlled energization", "stop_conditions": ["unexpected heat, smell, smoke, arcing, motion, damaged insulation, unstable supply, or measurement outside expected bounds", "missing G1 for independent energization", "source, protection, conductor, probe, or stored-energy value outside the approved activity envelope"]},
            {"id": "L2", "scope": "approved energy-limited extra-low-voltage work; planning caps normally no more than 24 V DC, 2 A, and 48 W plus soldering, bounded low-energy motors, protected batteries, and bench instruments, all pending institutional safety review and activity-specific confirmation", "authorization_gate": "current G1 plus task qualification; G3 certifies independent soldering progression but is not required for first direct-supervised solder exposure after an L2 task briefing", "supervision": "at least one trained instructor or assistant per 12 learners; direct supervision for first exposure and where the activity risk assessment requires it", "stop_conditions": ["any permit, guard, extraction, PPE, current limit, fuse, probe rating, or supervision failure", "unexpected thermal, electrical, chemical, battery, or rotating behavior", "configuration outside the approved L2 activity envelope"]},
            {"id": "L3", "scope": "institution-controlled higher fault-energy DC, converters, battery packs, accessible thermal or rotating hazards, chemicals or coatings, or lawful RF transmission outside L2 but not assigned to L4", "authorization_gate": "activity-specific risk assessment, current task authorization, and instructor sign-off", "supervision": "qualified local controller; at least one trained controller per 8 learners for unfamiliar L3 work, with direct supervision whenever required by the approved activity", "stop_conditions": ["any approval, permit, enclosure, interlock, PPE, extraction, legal condition, emergency control, or supervision failure", "energy, temperature, movement, exposure, field, or measurement outside the approved bounds"]},
            {"id": "L4", "scope": "mains, high voltage, high-energy storage, lasers, or safety-critical equipment under an adopted institutional procedure", "authorization_gate": "named institutional approval and task permit; advanced course only", "supervision": "continuous direct supervision by qualified personnel under the adopted institutional procedure", "stop_conditions": ["any deviation from the approved L4 procedure, permit, equipment, environment, or direct-supervision condition", "any interlock, protective-earth, isolation, enclosure, emergency-response, or communication failure"]},
        ], "prohibited_self_study": ["mains", "high-energy battery", "RF power", "high voltage"], "incident_owner": "institutional-safety-lead-pending-appointment"},
...
def course_manifests() -> tuple[dict, dict]:
    ...
    units = {"schema_version": "1.0.0", "version": VERSION, "status": "draft", "units": [{
        ...
        "platform_binding": "concept-stable", "localization_layer": "universal-core", "context_ids": [], "claim_register_ids": [claim["id"] for claim in ese111_ch04_claim_records()], "claim_review_due": None,
